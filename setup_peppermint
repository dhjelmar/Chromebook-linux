...
# let installation automatically create partitions with box checked to create swap file
#   - created new GPT partition table
#   - created 300 MB partition for EFI boot (format fat32; mount point /boot/EFI)
#     (I may have made this 100 MB but OK)
#   - created 29 GB partition for root and personal files (format ext4)

# above is not ideal if had bigger hard drive but it is good for tiny system
...
## hit enter twice after above command (use default id_ed25519 so ssh-add is simpler to use)
## copy the contents of the ~/.ssh/*pub key and add it to GitHub

## manually add ssh key to ssh agent
#  if used some unique name, then need to specify full path
#       ssh-add ~/.ssh/id_ed25519_github
# if used default name, then can just enter
ssh-add

## will need to run the above every time you log in
## instead, try following to make it persistent but may need to enter password each time
echo "AddKeysToAgent yes" > ~/.ssh/config2
...
##              full        left              right
##    top       Super+Up    Shift+Super+Left  Shift+Super+Right
##    bottom    Super+Down  Shift+Ctrl+Left   Shift+Ctrl+Right
##    left      Super+Left
##    right     Super+Right
#
# delete key
...
# install flatpak version of Thunderbird
...
# determine VERSION_CODENAME and add corresponding repository
cat /etc/os-release | grep VERSION_CODENAME
# I have bookworm (Debian 12) installed

# https://ahmorris.org/post/2023-09-16-install-rstudio-on-debian-12-bookworm/
sudo apt install r-base r-base-dev

# check posit website for RStudio releases and download
# https://posit.co/download/rstudio-desktop/

cd ~/Downloads
sudo apt install libssl-dev libclang-dev
sudo dpkg -i rstudio-2025.09.1-401-amd64.deb


#####
# FOLLOWING WAS NOT DONE
#####
